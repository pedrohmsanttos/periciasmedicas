...
$situacaoId = "";
if(!empty($this->request->data['Atendimento']['situacao_id'])){
    $situacaoId = $this->request->data['Atendimento']['situacao_id'];
}
echo $this->Form->hidden('situacao_bd', array('id' => 'situacao_bd', 'value' => $situacaoId));


// pr($situacaoId);die;

if ($tipologiaId == TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO){
    $tipoDoencaCat = ($this->request->data['AgendamentoCAT']['tipo_acidente_doenca']);
    // var_dump($tipoDoencaCat);die;
}
...
<?php if (TIPOLOGIA_ISENCAO_CONTRIBUICAO_PREVIDENCIARIA === $tipologiaId):
    ...
    <div class="row">
    <!-- aqui! -->       
        <div class="col-md-2 input-isencao">
            <?php
            echo
...
<div class="row">
    ...
    <div class="col-md-3">
        ...
        if(!in_array($tipologiaAgendamento,array(TIPOLOGIA_DESIGNACAO_DE_ASSISTENTE_TECNICO))){
            ...
            // $situacoes_ordenada = $situacoes[0];
            // var_dump($situacoes_ordenada);
            $this->Form->input('Atendimento.situacao_id', array('div' => array('class' => 'form-group'),
                'class' => 'form-control',
                'options' => $situacoes,
                'id' => 'inputSituacao',
                'selected' => $situacaoId,
    ...
    ?> 
        ...
        <?php if((isset($tipoDoencaCat) && ($tipoDoencaCat == 0)) || (empty($tipoDoencaCat))): 
        ?>
        <div class="col-md-1">
            <?= __('atendimento_chkbx_doenca'); ?>
            <?=

            $this->Form->checkbox('AtendimentoCAT.chkbx_doenca', array('label' => __('agendamento_chkbx_plantonista') . $isRequerid,
                'id' => 'chkbxDoenca',
                'label' => array('id' => 'labelDecorrente', 'text' => __('agendamento_chkbx_Doenca'). $isRequerid),
                'default' => 'f',
        ...
        ?>
            ...
            $this->Form->checkbox('AtendimentoCAT.chkbx_doenca', array('label' => __('agendamento_chkbx_doenca') . $isRequerid,
                'id' => 'chkbxDoenca',
                'label' => array('id' => 'labelDecorrente', 'text' => __('agendamento_chkbx_Doenca'). $isRequerid),
                'default' => 't',
                'checked' => 'checked',
...
if (!in_array($tipologiaId, array(
    ...
    <?php $labelCmpDtParecer = ($tipologiaId == TIPOLOGIA_ISENCAO_CONTRIBUICAO_PREVIDENCIARIA || $tipologiaId == TIPOLOGIA_RISCO_VIDA_INSALUBRIDADE || $tipologiaId == TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO) ? "A partir de" : "Data do diagnóstico da doença"; ?>
    <div class="row" id="rowDataParecerTecnico">
        <div class="col-md-3">
            <div class="form-group">
                <?php
                echo
                $this->Form->input('data_parecer', array('label' => $labelCmpDtParecer . $isRequerid,
                    'type' => 'text',
                    //'value' => $arrAtendimento['Atendimento']['data_parecer'],
                    'class' => 'inputData form-control', 'onkeyup' => 'JEffects.masks.data(this),formatoData(this)',
                    'onblur' => 'VerificaData(this,this.value)',
                    'onmouseout' => 'VerificaData(this,this.value)',
                    'disabled' => $formDisabled));
...
<div class="row">
    <div class="col-md-12">
        ...
        echo $this->Form->input('parecer', array('div' => array('class' => 'form-group'),
            ...
            'type' => 'textarea',
            'disabled' => $formDisabled,
            // 'value' => $arrAtendimento['Atendimento']['parecer'],
...
<script type="text/javascript">
    if( $.trim( $("#situacao_bd").val() ) != "" ){
        $("#inputSituacao").val($("#situacao_bd").val());
    }
</script>
