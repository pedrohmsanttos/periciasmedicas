...
ob_end_clean();
$filepathPart = WWW_ROOT. '/' .  'relatorio_personalizado';
if(!is_dir($filepathPart)) {
    mkdir($filepathPart);
}

$nomeArquivo = '/relatorio_' . date('Ymdhis') . '.pdf';
// die($filepathPart);
echo $tcpdf->Output($filepathPart . $nomeArquivo, 'F');
$url = Router::url('/', true).'relatorio_personalizado/' . $nomeArquivo;
