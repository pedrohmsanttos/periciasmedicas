...
class AgendamentoController extends BSController {
    ...
    function beforeFilter() {
        ...
        $this->Auth->allow('carregarHorariosAtendimentoMunicipioEndereco');
        $this->Auth->allow('checaDisponibilidade');
        $this->Auth->allow('testeGetHorarios');
        
        
    }

    public function checaDisponibilidade(){
        ...
        if($this->request->is(array('post', 'put'))){
            ...
            $idUnidade      = $this->request->data['unidade_atendimento_id'];
            $dataHoraAgend  = $this->request->data['data_hora'];
            $diaSemana      = $this->request->data['dia_semana'];
            $usuario_id     = $this->request->data['usuario_servidor_id'];
            $encaixe = null;
            ...
            $this->loadModel('AgendamentoDisponibilidade');

            $conditions = array(
                'AgendamentoDisponibilidade.data_inclusao > '      => date('Y-m-d') . ' 00:00:00',
                'AgendamentoDisponibilidade.data_inclusao < '      => date('Y-m-d') . ' 23:59:59',
                'usuario_servidor_id'   => $usuario_id,
                'dia_semana'            => $diaSemana
            );
            // pr($conditions);die;

            $agendDispo = $this->AgendamentoDisponibilidade->find('first', array(
            ...
            if(!empty($agendDispo)){
                $data['id'] = $agendDispo['AgendamentoDisponibilidade']['id'];
            }

            $data['data_hora']              = $dataHoraAgend;
            $data['tipologia_id']           = $idTipologia;
            $data['unidade_atendimento_id'] = $idUnidade;
            $data['usuario_servidor_id']    = $usuario_id;
            $data['dia_semana']             = $diaSemana;

            if($this->AgendamentoDisponibilidade->save($data)){
                echo "ok";
            }else{
                echo "error";
            }
    ...
    public function testeHorarios(){
        
        $data = date("Y-m-d h:i");
        pr($this->getHorarioByTipologia('11', $data));die;
    }

    public function getTempoTipologia($idTipologia){
        $this->loadModel('TempoConsultaAtendimento');
        $this->loadModel('ParametroGeral');
        $interConsultaAux   = $this->TempoConsultaAtendimento->buscarTempoConsultaAtendimento($idTipologia);

        if(is_null($interConsultaAux)){
            $interConsultaAux = $this->ParametroGeral->buscarIntervaloConsulta();
        }

        return $interConsultaAux;
    }

    public function getHorarioByTipologia($idTipologia, $dataHoraInicial){
    
        $interConsultaAux = $this->getTempoTipologia($idTipologia);

        $dataAntiga = Util::toDBDataHora($dataHoraInicial);
        $dataAntiga = date("Y-m-d H:i", strtotime($dataAntiga));
        $endTime = strtotime("+$interConsultaAux minutes", strtotime($dataAntiga));
    ...
    public function tipologiaExisteAgendaMedica($idTipologia, $agendasMedicas){
        $retorno = false;

        if(!empty($agendasMedicas)){
            foreach($agendasMedicas as $agenda){
                if (in_array($idTipologia, $agenda['tipologias'])) {
                    $retorno = true;
                    break;
                }
            }
    ...
    public function getChavesHorarioByPeriodo($periodoAgendamento, $horarios, $chavesInicio, $idTipologia){


        // PORTO

        $horarios = $this->hashByDia($horarios);
       
        $chaves = array();
        if(!empty($horarios)){

            $diaInicioAgendamento   = date('Y-m-d', strtotime($periodoAgendamento['inicio']));
            $horariosDia = $horarios[$diaInicioAgendamento];

            if(!empty($chavesInicio)){
                foreach($chavesInicio as $chave){
                    unset($horariosDia[$chave]);
                }
            }
            ...
            // PORTO
            $chaveAnterior = 0;
            foreach($horariosDia as $cha => $horDia){

                // Garantindo que vai remover a chave anterior, caso o horario que vai ser atendido seja maior que o percorrido
                if( strtotime($horDia)  < strtotime($periodoAgendamento['inicio']) ){
                   
                    $interConsultaAux = $this->getTempoTipologia($idTipologia);

                    $dataAntiga = Util::toDBDataHora($horDia);
                    $dataAntiga = date("Y-m-d H:i", strtotime($dataAntiga));
                    $dataSomada = strtotime("+$interConsultaAux minutes", strtotime($dataAntiga));

                    if($dataSomada > strtotime($periodoAgendamento['inicio'])){
                        $chaves[] = $cha;
                        $achouUmInicio = true;
                    }
                }
                ...
                if(strtotime($periodoAgendamento['inicio']) == strtotime($horDia)   ){
                    $chaves[] = $cha;
                    $achouUmInicio = true;
                }

                if($achouUmInicio && strtotime($horDia) < strtotime($periodoAgendamento['final'])){
                    $chaves[] = $cha;
                }

                if(strtotime($horDia) > strtotime($periodoAgendamento['final']) && $achouUmInicio){
                    break;
                }
    ...
    public function hashByDia($agenda){
        
        $retornoHorario = array();
        foreach($agenda as $chave => $horario){
            $retornoHorario[date('Y-m-d', strtotime($horario))][$chave] = $horario;
        }
        
        return $retornoHorario;
    }

    public function getHorariosByAgendamento($agendamentos, $agendasMedicas, $idTipologiaAgendamento,$retornoHorarios){
        // PEDRO_SANTOS
        $agendas        = array();
        $tipologias     = array();
        $chavesRemover  = array();
		...
		$duracao  = $this->getTempoTipologia($idTipologiaAgendamento);

        foreach($agendasMedicas as $Perito => $agenMedica){
          
            $agendas = $agenMedica['horarios'];
            $tipologias = array_merge($tipologias, $agenMedica['tipologias']);
            $perito=$Perito;
            
            foreach ($agendas as $ihorario => $horario) {
                $data = date('Y-m-d', strtotime($horario));
                $hora = date('H', strtotime($horario));
                $minuto = date('i', strtotime($horario));
				$idin = array();
				$indice = $minuto;
				while($indice <= ($duracao+$minuto) ){
					if($indice == ($duracao+$minuto)){
						$indice=$indice - 1;
					}
					$idin[]=$indice;
					$indice= $indice + 5;
                ...
                $this->loadModel('AgendamentoControle');

                $joins = array(
                    'table' => 'agendamento',
                    'alias' => 'Agendamento',
                    'type' => 'inner',
                    'conditions' => array('Agendamento.id = AgendamentoControle.agendamento_id  ')
                );

                $conditions = array(
                    'AgendamentoControle.perito_id = '      => $perito,
                    'AgendamentoControle.data  = '      => $data,
                    'AgendamentoControle.hora  = '   => $hora,
                    'AgendamentoControle.minuto in '            => $idin,
                    'Agendamento.ativo ='            => true
                );
                

                $agendControle = $this->AgendamentoControle->find('first', array(
                    'joins' => array($joins),
                    'conditions' => array( $conditions ),
                ));
                
                if(isset($agendControle) && !empty($agendControle)){
        ...
        return $chavesRemover;
    }

    public function getAllHorarios($agendasMedicas){
        $retornoHorarios    = array();

        foreach($agendasMedicas as $agenMedica){
            $retornoHorarios = array_merge($retornoHorarios, $agenMedica['horarios']);
        }

        return $retornoHorarios;
    }

    public function removeListaAgendamentosHorariosNovo($agendamentos, $agendasMedicas, $idTipologia){
        ...
        $retornoHorarios = $this->getAllHorarios($agendasMedicas);

        if(!empty($agendamentos) && !empty($agendasMedicas)){
            
            $chavesRemover = $this->getHorariosByAgendamento($agendamentos, $agendasMedicas,$idTipologia,$retornoHorarios);
        }

        if(!empty($chavesRemover)){
            foreach($chavesRemover as $chave){
                unset($retornoHorarios[$chave]);
            }
        }
        return $retornoHorarios; 
    ...
    public function removeListaAgendamentosHorarios($agendamentos, $agendasMedicas){
        
         
        $chavesRemover = array();
        $chavesRemover = $this->getHorariosByAgendamento($agendamentos, $agendasMedicas);

        foreach ($agendamentos as $agendamento) {
            foreach ($agendasMedicas as $key => $agendaMedica){
                $dataAgendamento = Util::toDBDataHora($agendamento['Agendamento']['data_hora']);
                $idTipologia = $agendamento['Agendamento']['tipologia_id'];
                ...
                if (in_array($dataAgendamento, $agendaMedica['horarios'])) {
                    ...
                    if (in_array($idTipologia, $agendaMedica['tipologias'])) {
                        ...
                        if(!empty($chavesRemover)){
                            foreach($chavesRemover as $chave){
                                unset($agendasMedicas[$key]['horarios'][$chave]);
                            }
                        }
                    }
                }
    ...
    public function carregarHorariosAtendimento() {
        if ($this->request->is(array('post', 'put'))) {
            ...
            // PORTO
            $horariosDisponiveis = $this->getHorariosDisponiveis($idTipologia, $idUnidade, $diaSemana, $encaixe);
    ...
    public function carregarHorariosAtendimentoDomicilio() {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $horariosDisponiveis = $this->getHorariosDisponiveisDomicilio($idTipologia, $idUnidade, $diaSemana, $encaixe);
    ...
    public function getHorariosDisponiveis($idTipologia, $idUnidade, $diaSemana, $encaixe, $agendamentoTemp = null){
        ...
        foreach ($arrAgendaSistema as $agendaSistema){
            ...
            if(!is_null($agendamentoTemp) && !empty($agendamentoTemp)){
                foreach($agendamentoTemp as $agTem){
                    $agendamentos[] = $agTem;
                }
            }
            ...
            $qtdAgendaAtendimento = count($agendasAtendimentos);

            // Se so tiver uma agenda de atendimento
                // Salvar num array todos os horarios disponíveis para atendimento (somando com o tempo de consulta)
            if($qtdAgendaAtendimento == 1){

                if(!empty($agendamentos)){

                    $ultimoAgend = end($agendamentos);

                    foreach ($agendamentos as $agendamento) {
                        if($idTipologia != $agendamento['Agendamento']['tipologia_id']){
                    
                            $horariosAgendAnte[] =  $this->getHorarioByTipologia($idTipologia,$agendamento['Agendamento']['data_hora']);

                        }
                    }

                    $ultimoHorarioDisponivel = null;

                    if($idTipologia != $ultimoAgend['Agendamento']['tipologia_id']){
                        $ultimoHorarioDisponivel = $this->getHorarioByTipologia($idTipologia,$agendamento['Agendamento']['data_hora']);
                    }
            ...
            if (is_null($encaixe) || $encaixe == "false") {
                ...
                // PORTO
                $agendasMedicas = $this->removeListaAgendamentosHorariosNovo($agendamentos, $agendasMedicas, $idTipologia);

                
                
            }else{
                $agendasMedicas = $this->getAllHorarios($agendasMedicas);
            }

            //Adiciona todos os horários disponíveis na agenda em uma única lista
            foreach ($agendasMedicas as $horario) {
    ...
    public function getHorariosDisponiveisMunicipioEndereco($idTipologia, $diaSemana, $encaixe, $cids, $idMunicipio){
        ...
        foreach ($ids as $idUnidade) {
            ...
            foreach ($arrAgendaSistema as $agendaSistema){
                //Consulta todas as agendas dos peritos que atenda a tipologia, unidade e dia da semana selecionados
                $agendasAtendimentos = $this->AgendaAtendimentoDomicilio->consultarAgendasAtendimento($idTipologia, $idUnidade, $diaSemana, $agendaSistema);
                ...
                $agendasMedicas = $this->montarListaHorariosDomicilio($intervaloConsulta, $agendasAtendimentos, $diaSemana, $agendaSistema['AgendaSistema']);
                ...
                foreach ($agendasMedicas as $horario) {
                        ...
                        if( isset($qtdAgendaAtendimento) && $qtdAgendaAtendimento == 1 && !empty($ultimoHorarioDisponivel)){
                            if($horario > $ultimoHorarioDisponivel){
                                $horariosDisponiveis[] = $horario;
                            }
                        }else{
                            $horariosDisponiveis[] = $horario;
    ...
    public function testeGetHorarios(){
        ...
        if($this->request->is('post')){
            ...
            $idUnidade      = $this->request->data['unidade_id'];
            $diaSemana      = $this->request->data['dia_semana'];
            $dataHoraAgendamento = $this->request->data['data_hora'];

            // pr( $this->getHorariosAgendamentos($idTipologia, $idUnidade, $diaSemana) );die;

            // pr( $this->getPeriodoAgendamentoByHorario($idTipologia,$dataHoraAgendamento) );die;

            pr( $this->getPeritoAtendimento($idTipologia, $idUnidade, $diaSemana,$dataHoraAgendamento) );die;
    ...
    public function getPeritoAtendimento($idTipologia, $idUnidade, $diaSemana, $dataHoraAgendamento, $atendimentoDomiciliar = null){

        
        $agendasMedicas = $this->getHorariosAgendamentos($idTipologia, $idUnidade, $diaSemana,$atendimentoDomiciliar);
        ...
        $aux = 0;
        if( !empty( $agendasMedicas ) ){
            foreach($agendasMedicas as $idPerito => $agenda){
             
                if( in_array($dataHoraAgendamento, $agenda['horarios']) && in_array($idTipologia, $agenda['tipologias']) ){
                    
                    $aux++;

                    foreach($agenda['horarios'] as $horario){
                        
                        if( strtotime( $horario ) == strtotime( $dataHoraAgendamento)){

                            // Verificando se o perito ja está sendo alocado para outro atendimento
                            
                            $conditions = array('AND' =>
                                array(
                                    array('AgendamentoDisponibilidade.unidade_atendimento_id'   => $idUnidade),
                                    array('AgendamentoDisponibilidade.tipologia_id'             => $idTipologia),
                                    array('AgendamentoDisponibilidade.dia_semana'               => $diaSemana),
                                    array('AgendamentoDisponibilidade.perito_id'                => $idPerito),
                                    array('AgendamentoDisponibilidade.agendamento_id'           => ""),
                                    array(
                                        'OR' => array(
                                            array('AND' => array(
                                                    array('AgendamentoDisponibilidade.data_hora <= ' => $dataHoraAgendamento),
                                                    array('AgendamentoDisponibilidade.termino >= ' => $this->getPeriodoAgendamentoByHorario($idTipologia,$dataHoraAgendamento)['final']),
                                                )
                                            ),
                                            array('AND' => array(
                                                    array('AgendamentoDisponibilidade.data_hora >= ' => $dataHoraAgendamento),
                            ...
                            if(!is_null($atendimentoDomiciliar)){
                                $conditions['AND'][] = array('AgendamentoDisponibilidade.atendimento_domiciliar' => true);
                            }

                            

                            $agendamentoDispo = $this->AgendamentoDisponibilidade->find('first', array(
                                'conditions' => $conditions
                                )
                            );
                            
                            if(!empty($agendamentoDispo)){
                                $peritos[] = $idPerito;
                                continue;
                            }else{
                                $retornoPerito = $idPerito;
                                $peritos[] = $idPerito;
                                break;
                            }
        ...
        $retorno['total']   = $aux;
        $retorno['perito']  = $retornoPerito;
        $retorno['peritos']  = $peritos;
        return $retorno;
        
    }

    public function getHorariosAgendamentos($idTipologia, $idUnidade, $diaSemana, $atendimentoDomiciliar = null){
        ...
        foreach ($arrAgendaSistema as $agendaSistema){
            ...
            if(!is_null($atendimentoDomiciliar)){
			   ...
			   foreach($agendasAtendimentos as $key => $agdAtend){
                    if(empty($agdAtend['Tipologia'])){
                        $db = ConnectionManager::getDataSource('default');
                        $sqlTipologia = "select t.* from desen.agen_aten_domic_tip aadt inner join desen.tipologia as t on t.id = aadt.tipologia_id where aadt.agend_atendi_domic_id  = 76";
                        $ret = $db->fetchAll($sqlTipologia);
						
						$tipologias = array();
						foreach($ret as $tipo){
							$tipologias[] = $tipo[0];
						}
						$agendasAtendimentos[$key]['Tipologia'] = $tipologias;
                    }
                }
            }
    ...
    public function getHorariosDisponiveisDomicilio($idTipologia, $idUnidade, $diaSemana, $encaixe){
    ...
    public function carregarHorariosAtendimentoMunicipioEndereco() {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            if(isset($this->request->data['cids']) && !empty($this->request->data['cids'])){
            ...
            $horariosDisponiveis = $this->getHorariosDisponiveisMunicipioEndereco($idTipologia,$diaSemana,$encaixe,$cids,$idMunicipio);
    ...
    public function getPeriodoAgendamento($agendamento){
        $aux = array();
        if(!empty($agendamento)){
            $aux['inicio']          = Util::toDBDataHora($agendamento['Agendamento']['data_hora']);
            $aux['final']           = $this->getHorarioByTipologia($agendamento['Agendamento']['tipologia_id'],$agendamento['Agendamento']['data_hora']);
            $aux['tempo_consulta']  = $this->getTempoTipologia($agendamento['Agendamento']['tipologia_id']);
        }
        return $aux;
    } 

    public function getPeriodoAgendamentoByHorario($tipologiaId,$dataHoraAgend){
        $aux = array();
        if(!empty($dataHoraAgend)){
            $aux['inicio']          = $dataHoraAgend;
            $aux['final']           = $this->getHorarioByTipologia($tipologiaId,$dataHoraAgend);
            $aux['tempo_consulta']  = $this->getTempoTipologia($tipologiaId);
        }
        return $aux;
    } 

    public function listaHorariosAgendamentoMarcados($agendamentos){
        $retornoPeriodosAgendamentos = array();
        if(!empty($agendamentos)){
            foreach($agendamentos as $agendamento){
                $aux = array();
                $aux['inicio']  = Util::toDBDataHora($agendamento['Agendamento']['data_hora']);
                $aux['final']   = $this->getHorarioByTipologia($agendamento['Agendamento']['tipologia_id'],$agendamento['Agendamento']['data_hora']);
                $retornoPeriodosAgendamentos[] = $aux;
            }
        }
        return $retornoPeriodosAgendamentos;
    } 
    ...
    public function montarListaHorariosDomicilio($intervaloConsulta, $agendasAtendimentos, $diaSemana, $agendaSistema) {
        //die("aq");
        ...
        $this->loadModel("AgendaSistema");
        $agendasSistemas = $this->AgendaSistema->find("all", array(
            'conditions' => $conditions
        ));

        $log = $this->AgendaSistema->getDataSource()->getLog(false, false);
        pr($agendasSistemas);die;
        pr($log);die;*/
        ...
        foreach ($agendasAtendimentos as $agendaAtendimento) {
            ...
            $horariosMedico = [];
            $hora_inicial = $agendaAtendimento['AgendaAtendimentoDomicilio']['hora_inicial'];
            $hora_final = "";
            if(isset($agendaAtendimento['AgendaAtendimentoDomicilio']['hora_final'])){
                $hora_final = $agendaAtendimento['AgendaAtendimentoDomicilio']['hora_final'];
            }
            $hora_agenda = "";

            if(isset($agendaSistema['AgendaSistema']['hora'])){
                $hora_agenda = $agendaSistema['AgendaSistema']['hora'];
            }
            
            if ( (!empty($hora_inicial) && !empty($hora_agenda)) && ($hora_inicial != $hora_agenda) ) {
                $horas = $this->criarHorariosAgendaAtendimento($agendaSistema['AgendaSistema']['hora'], $agendaAtendimento['AgendaAtendimentoDomicilio']['hora_final'], $intervaloConsulta);  
            }else{
            ...
            //pr($horas);die;
            foreach ($horas as $hora) {
                $horariosMedico = array_merge($horariosMedico, $this->formatarHorarioAtendimento($hora, $diaSemana, $agendaSistema, true));
    ...
    private function formatarHorarioAtendimento($hora, $diaSemana, $agendaSistema = null, $domicilio = false) {
        ...
        if($domicilio == false){
            ...
            for ($i = 0; $i < 10; $i++){    
                ...
                $dataCompare = date("Y-m-d", strtotime($dataAtual));
                $dataAgenda = date("Y-m-d", strtotime($agendaSistema["prazo_final"]));

                if($dataCompare <= $dataAgenda){
    ...
    public function visualizar($id = null) {
        ...
        if(empty($agendamento)){
            $this->Session->setFlash(__('Esse agendamento foi excluído!'), 'flash_alert');
            return $this->redirect(array('action' => 'index'));
        }
        
        $this->set("formDisabled", true);
        $this->set("isVisualizar", true);
    ...
    */
    ...
    public function adicionar(){
        ...
        if ($this->request->is('post')) {
            ...
            $atendimentoUndProxima = "";
            $municipioAtend        = "";
            $unidadeAtendId        = "";
            $diaSemana             = "";
            $municipioEndere       = "";
            $atendimentoDomicEnde  = "";

            $this->loadModel("Endereco");

            if(isset($this->request->data['Agendamento']['atendimento_domiciliar'])){
                $atendimentoDomiciliar = $this->request->data['Agendamento']['atendimento_domiciliar'];
            }

            if( !empty($this->request->data['Agendamento']['atend_domicilio_unidade_proxima']) ){
                $atendimentoUndProxima 	= $this->request->data['Agendamento']['atend_domicilio_unidade_proxima'];
            }

            if( !empty( $this->request->data['Agendamento']['municipio_id_atend_domicilio'] ) ){
                $municipioAtend 	= $this->request->data['Agendamento']['municipio_id_atend_domicilio'];
            }

            if( !empty( $this->request->data['Agendamento']['unidade_atendimento_id'] ) ){
                $unidadeAtendId		= $this->request->data['Agendamento']['unidade_atendimento_id'];
            }

            if(!empty( $this->request->data['Agendamento']['dia_semana'] )  ){
                $diaSemana		= $this->request->data['Agendamento']['dia_semana'];
            }
           
            if( !empty( $this->request->data['EnderecoAtendimentoDomicilio']['municipio_id'] ) ){
                $municipioEndere = $this->request->data['EnderecoAtendimentoDomicilio']['municipio_id'];
            }	
            
            if( !empty( $this->request->data['EnderecoAtendimentoDomicilio']['id']  ) ){
                $enderecoId = $this->request->data['EnderecoAtendimentoDomicilio']['id'];
                $endereco = $this->Endereco->findById($enderecoId);
                $atendimentoDomicEnde = $endereco['Endereco']['municipio_id']; 
                
            }
            
            
            $atendimentoDomic = false;
            $atendimentoEnder = false;

            

            if( !empty($atendimentoDomiciliar) && $atendimentoUndProxima == "1" && !empty($municipioAtend) && !empty($diaSemana)){
                $atendimentoDomic = true;
                
            }

            if( !empty($atendimentoDomiciliar) && (!empty($municipioEndere) || !empty($atendimentoDomicEnde))  && empty($atendimentoUndProxima) && !empty($diaSemana)){
                $atendimentoEnder = true;
            ...
            $idTipologia    = $this->request->data['Agendamento']['tipologia_id'];
            $idUnidade      = $this->request->data['Agendamento']['unidade_atendimento_id'];
            $dataHoraAgend  = Util::toDBDataHora($this->request->data['Agendamento']['data_hora']);
            $diaSemana      = $this->request->data['Agendamento']['dia_semana'];
            $usuario_id     = $this->request->data['Agendamento']['usuario_servidor_id'];
            //$encaixeReq     = $this->request->data['Agendamento']['encaixe'];
            //pr($this->request->data['Agendamento']);
            //die;
            if(isset($this->request->data['Agendamento']['encaixe'])){
                $encaixeReq     = $this->request->data['Agendamento']['encaixe'];
            }
            else{
                $encaixeReq     = 0;
            ...
            // Consultando tabela auxiliar para validar a questão dos horários
            ...
            $tipologiasSemHorario = array(
                TIPOLOGIA_APOSENTADORIA_ESPECIAL,
                TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO,
                TIPOLOGIA_INSPECAO,
                TIPOLOGIA_RISCO_VIDA_INSALUBRIDADE,
                TIPOLOGIA_SINDICANCIA_INQUERITO_PAD
            );
            ...
            $agendamento_disponibilidade_id=0;
                $this->loadModel("AgendaSistema");
                $this->loadModel("AgendaAtendimento");
                ...
                $idTipologia    = $this->request->data['Agendamento']['tipologia_id'];
                $idUnidade      = $this->request->data['Agendamento']['unidade_atendimento_id'];
                $dataHoraAgend  = $this->request->data['Agendamento']['data_hora'];
                $diaSemana      = $this->request->data['Agendamento']['dia_semana'];
                $usuario_id     = $this->request->data['Agendamento']['usuario_servidor_id'];
                ...
                $okSave             = false;
                $exigenciaInvalida  = false;
            

                // ------------------ Verificando se o horário marcado está disponível --------------------- //
                $encaixe = "false";
                if(isset($this->request->data['Agendamento']['encaixe'])){
                    if($this->request->data['Agendamento']['encaixe'] == "1"){
                        $encaixe = "true";
                    }
                }
                $this->loadModel("AgendamentoDisponibilidade");

                $horarios = $this->getHorariosDisponiveis($idTipologia, $idUnidade, $diaSemana, $encaixe);

                

                if( $atendimentoDomic ){
                    $horarios = $this->getHorariosDisponiveisDomicilio($idTipologia, $idUnidade, $diaSemana, $encaixe);
                }

                if($atendimentoEnder){
                    $cids = $this->request->data['Cids'];
                    
                    if(empty($municipioEndere)){
                        $municipioEndere = $atendimentoDomicEnde;
                    }
                    $horarios = $this->getHorariosDisponiveisMunicipioEndereco($idTipologia, $diaSemana, $encaixe, $cids, $municipioEndere);
                    
                }

                $dataHoraAgendamento = $this->request->data['Agendamento']['data_hora'];
                $dataHoraAgendamento = date('Y-m-d H:i', strtotime( str_replace("/", "-", $dataHoraAgendamento)  ));
                
                if( (!isset($encaixeReq) || $encaixeReq == "0") && !in_array($tipologiaSelecionada,$tipologiasSemHorario)){
                    if( !in_array($dataHoraAgendamento, $horarios) ){
                ...
                // ------------------ END - Verificando se o horário marcado está disponível --------------------- //

                //echo "<br>AAAAA<br>";
               // echo "<br>".$idTipologia.",". $idUnidade.",". $diaSemana.",".$dataHoraAgend."<br>";
 
                $dataHoraAgend  = Util::toDBDataHora($this->request->data['Agendamento']['data_hora']);

                if( (!isset($encaixeReq) || $encaixeReq == "0")  && $horarioPreenchido == false && !in_array($tipologiaSelecionada,$tipologiasSemHorario)){
                    //echo "<br>BBB<br>";
                    $domiciliar = null;

                    if(!empty($atendimentoDomiciliar)){
                        $domiciliar = $atendimentoDomiciliar;
                    }
                    // PEDRO_SANTOS
                    $retornoAgenda = $this->getPeritoAtendimento($idTipologia, $idUnidade, $diaSemana, $dataHoraAgend,$domiciliar);
						 
                    //echo "<br>CCCC<br>";
                   
                
                    if( intval($retornoAgenda['total']) == 0 || empty($retornoAgenda['perito']) || empty($retornoAgenda['peritos']) ) {
                        $okSave = false;    
                        $this->Session->setFlash("Não existe perito disponível para atender nesse horário!", 'flash_alert');                   
                    }else{
                        $conditions = array('AND' =>
                            array(
                                array('AgendamentoDisponibilidade.unidade_atendimento_id'   => $idUnidade),
                                ...
                                array(
                                    'OR' => array(
                                        array('AND' => array(
                                                array('AgendamentoDisponibilidade.data_hora <= ' => $dataHoraAgend),
                                                array('AgendamentoDisponibilidade.termino >= ' => $this->getPeriodoAgendamentoByHorario($idTipologia,$dataHoraAgend)['final']),
                                            )
                                        ),
                                        array('AND' => array(
                                                array('AgendamentoDisponibilidade.data_hora >= ' => $dataHoraAgend),
                        ...
                        $agendamentoDispo = $this->AgendamentoDisponibilidade->find('all', array(
                            'conditions' => $conditions
                            )
                        );
                        
                
                        if(!empty($agendamentoDispo) && count($agendamentoDispo) >= intval($retornoAgenda['total'] )){
                            $okSave = false;    
                            $this->Session->setFlash("Já existem agendamento(s) para esse horário. Escolha outro!", 'flash_alert'); 
                        }else{
                
                            $dataTabAux = array();
                
                            $dataTabAux['data_hora']                  = $dataHoraAgend;
                            $dataTabAux['tipologia_id']               = $idTipologia;
                            $dataTabAux['unidade_atendimento_id']     = $idUnidade;
                            $dataTabAux['dia_semana']                 = $diaSemana;
                            $dataTabAux['perito_id']                  = $retornoAgenda['perito'];
                            $dataTabAux['termino']                    = $this->getPeriodoAgendamentoByHorario($idTipologia,$dataHoraAgend)['final'];
                            
                            if(!empty($atendimentoDomiciliar)){
                                $dataTabAux['atendimento_domiciliar']  = true;
                            }

                            $ds = $this->AgendamentoDisponibilidade->getdatasource();

                            $ds->begin();
                            if($this->AgendamentoDisponibilidade->save($dataTabAux)){
                                $ds->commit();
                                $agendamento_disponibilidade_id=$this->AgendamentoDisponibilidade->id;
                ...
                $numExigencia   = $this->request->data['Agendamento']['num_exigencia'];
                $checkExigencia = $this->request->data['Agendamento']['chkbx_exigencia'];
                $idUsuario      = $this->request->data['Agendamento']['usuario_servidor_id'];

                if($checkExigencia == "1" && !empty($numExigencia)){
                    // die("aqui");
                    $this->loadModel('Atendimento');
                    $this->loadModel('Tipologia');
                    ...
                    $condicoes['(cast ("Atendimento".id as VARCHAR))'] = $numExigencia;
                    $condicoes['Atendimento.situacao_id'] = 1;
                ...
                //$idUnidade      = $this->request->data['Agendamento']['unidade_atendimento_id'];
                //$dataHoraAgend  = $this->request->data['Agendamento']['data_hora'];
                $dataAgend = date('Y-m-d', strtotime($dataHoraAgend));
                $horaAgend = date('H', strtotime($dataHoraAgend));
                $mintAgend = date('i', strtotime($dataHoraAgend));
                $durAgend  = $this->getTempoTipologia($idTipologia);
                $mintFinalAgend = intval($mintAgend) + intval($durAgend);
                
                //$diaSemana      = $this->request->data['Agendamento']['dia_semana'];
                //$usuario_id     = $this->request->data['Agendamento']['usuario_servidor_id'];
                $peritoId       = "";
                if(!empty($retornoAgenda['perito'])){
                    $peritoId       = $retornoAgenda['perito'];
                }
                ...
                if(empty($peritoId)){
                    ...
                    $this->Session->setFlash("Não existe(m) perito(s) para atender esse horário!", 'flash_alert');
                    $messageError="Não existe(m) perito(s) para atender esse horário!";
                }

               // echo "<br>1<br>";
                 // PORTO
                //$idTipologia, $peritoId, '$dataAgend', $horaAgend, $mintAgend, $mintFinalAgend, $durAgend, now()
                $db = ConnectionManager::getDataSource('default');
                
                $deuErro = false;

                $atendimentoDom = false;
                if(!empty($atendimentoDomiciliar)){
                   $atendimentoDom  = true;
                }
				
                // Verificar unicidade do horário apenas se não for encaixe
                if( (!isset($encaixeReq) || $encaixeReq == "0") && !in_array($tipologiaSelecionada,$tipologiasSemHorario)){
                   
                    if(isset($retornoAgenda)){
                  

                        foreach($retornoAgenda['peritos'] as $perito) {
                        ...
                        //   $peritoId = "";
                          ...
                          try{
                              ...
                              //$queryInsert = "INSERT INTO desen.agendamento_auxiliar (tipologia_id, perito_id, data, hora, minuto, minuto_final, duracao, data_inclusao) VALUES($idTipologia, $peritoId, '".$dataAgend."', $horaAgend, $mintAgend, $mintFinalAgend, $durAgend, now());";
                              $queryInsert = "SELECT desen.insere_agendamento_controle($idTipologia, $peritoId, CAST('".$dataAgend."' AS date), $horaAgend, $mintAgend,  $durAgend, $atendimentoDom);";
      
                              //$ret = $db->rawQuery($queryInsert);
                              $ret = $db->fetchAll($queryInsert);
      
                              $r = $ret[0][0]["insere_agendamento_controle"];
                ...
                if($deuErro){
                    ...
                    $horarioPreenchido = true;
                    $okSave = false;
                    $this->Session->setFlash("Já existe um agendamento marcado para esse horário!", 'flash_alert');
                    $messageError="Já existe um agendamento marcado para esse horário!";
					$this->request->data['Agendamento']['data_hora'] = "";

                    //echo "ERRO!!".$queryInsert;
                }
                else{
                  //pr($deuErro);die;
                  // die("ate aqi");
                  if($horarioPreenchido){
                      $this->Session->setFlash("Data e Hora do agendamento ja preenchido. Escolha outra!", 'flash_alert');
                      $messageError="Data e Hora do agendamento ja preenchido. Escolha outra!";
                  }else if($exigenciaInvalida){

                      $this->Session->setFlash("Número de Exigência inválido!", 'flash_alert');
                      $messageError="Número de Exigência inválido!";
                ...
                // o agendamento que está sendo cadastrado no momento, ja existe um range cadastrado pra ele? (está contido)

                 //die;
                //  pr(  $this->Agendamento);die;
                ...
                if( (!isset($encaixeReq) || $encaixeReq == "0" ) && !in_array($tipologiaSelecionada,$tipologiasSemHorario) ){
                    ...
                    // PEDRO_SANTOS
                    if($okSave && $agendamento_id !=0 && !empty($peritoId)){
                        try{
                            $idin = array();
                            $indice = $mintAgend;
                            $duracao= $durAgend;
                            while($indice <= ($duracao+$mintAgend) ){
                                if($indice == ($duracao+$mintAgend)){
                                    $indice=$indice - 1;
                                }
                                $idin[]=$indice;
                                $indice= $indice + 5;
                            ...
                            $m = implode(",",$idin);
                            $queryInsert = "UPDATE desen.agendamento_controle set agendamento_id=$agendamento_id, atendimento_domiciliar=$atendimentoDom where tipologia_id=$idTipologia AND  perito_id=$peritoId AND data=CAST('".$dataAgend."' AS date) AND hora= $horaAgend AND minuto in ($m) ;";
                            $db->rawQuery($queryInsert);
        
                            $queryInsert = "UPDATE desen.agendamento_disponibilidade set agendamento_id=$agendamento_id, atendimento_domiciliar=$atendimentoDom where id=$agendamento_disponibilidade_id ;";
                            $db->rawQuery($queryInsert);
                            ...
                            $deuErro = true;
                            $okSave = false;
                            $this->Session->setFlash("Ocorreu um erro ao tentar salvar o agendamento. Favor tentar novamente! ".$e->getMessage(), 'flash_alert');
                            $messageError="Ocorreu um erro ao tentar salvar o agendamento. Favor tentar novamente! ".$e->getMessage();
                            //echo "<pre>";
                            //var_dump($e->getMessage());
                            //echo "</pre>";
            ...
            if($okSave){
                ...
                $this->Session->setFlash($messageError, 'flash_alert');
        ...
        if(isset($okSave) && !$okSave){
            if($this->request->data['Agendamento']['tipologia_id'] == TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO){
                $mensagem = "Erro ao adicionar Agendamento - CAT: ";
                /*foreach ($this->Agendamento->validationErrors as $key => $value){
                    foreach ($value as $v){
                        $mensagem .= "\n";
                        $mensagem .= $v;
                        $mensagem .= "\n";
                ...
                foreach ($this->Agendamento->validationErrors as $key => $value){
                    foreach ($value as $v){
                        $this->Session->setFlash($v, 'flash_alert');
                    }
                }
    ...
    public function homologar($id, $retorno = null)
    {
        ...
        // Verificando se o AGENDAMENTO já foi homologado
        if(!in_array($agendamento['Agendamento']['homologa'], array('0','1'))){
            ...
            $this->Session->setFlash(__('Esse agendamento já foi homologado.'), 'flash_alert');
                return $this->redirect(array('controller' => 'Agendamento', 'action' => 'index'));
        }
    ...
    public function validateCamposCat(){
        ...
        if (isset($this->data[$this->alias]['tipologia_id'])) {
            ...
            if ($tipologia == TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO){
                ...
                if(empty($this->data['AgendamentoCAT']['tipo_acidente_doenca'])){
                    $this->invalidate('tipoacidenteMsg', 'É necessário preencher o tipo');

                }
                if(empty($this->data['AgendamentoCAT']['registro_policial_acidente_doenca'])){
                ...
                if(empty($this->data['AgendamentoCAT']['assistencia_medica_hospitalar_acidente_doenca'])){
                ...
                if(empty($this->data['AgendamentoCAT']['testemunha_acidente_doenca'])){
    ...
    private function enviarEmailInformandoSolicitacaoLicenca($destinatario, $nomeServidor, $cpfServidor, $tipologia = '', $cod_tipologia = false) {
        ...
        if($cod_tipologia == TIPOLOGIA_RISCO_VIDA_INSALUBRIDADE || $cod_tipologia == TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO){
            ...
            $htmlMsg .= "<br/>Acesse o Sistema SPM: www.irhspm.pe.gov.br ";
        }
        $htmlMsg .='</body>
        </html>';
    ...
    public function deletar($id, $retorno = false) {
        ...
        if ($this->request->is('get')) {
            ...
            $atendimento = $this->Atendimento->find('first', array(
                'conditions' => array('agendamento_id' => $id)
            ));

           
            $tipologiaAgendamento = $agendamento['Agendamento']['tipologia_id'];

            $tipologiasConfPres = array(
                TIPOLOGIA_APOSENTADORIA_ESPECIAL, 
                TIPOLOGIA_SINDICANCIA_INQUERITO_PAD,
                TIPOLOGIA_DESIGNACAO_DE_ASSISTENTE_TECNICO,
                TIPOLOGIA_INSPECAO
            );

            $tipologiasHomolog = array(
                TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO,
                TIPOLOGIA_RISCO_VIDA_INSALUBRIDADE
            );

            if(!empty( $atendimento )){
                $this->Session->setFlash("Exclusão não pode ser efetuada, pois existe um atendimento associado à esse agendamento", 'flash_alert');
                return $this->redirect(array('action' => 'index'));
            }

            else if(in_array($tipologiaAgendamento,$tipologiasHomolog) && !empty($agendamento['Agendamento']['homologa'])){
                $this->Session->setFlash("Exclusão não pode ser efetuada, pois esse agendamento está em processo de homologação", 'flash_alert');
                return $this->redirect(array('action' => 'index'));
            }

            else if($agendamento['Agendamento']['agendamento_confirmado'] && (!in_array($tipologiaAgendamento, $tipologiasHomolog))   ){
                $this->Session->setFlash("Exclusão não pode ser efetuada, pois esse agendamento já foi confirmado.", 'flash_alert');
                return $this->redirect(array('action' => 'index'));
            }
            ...
            if ($this->Agendamento->delete($id)){
                try{
                    $sqlDelete = "DELETE FROM agendamento_controle WHERE agendamento_id = " . $id;
                    $db->rawQuery($sqlDelete);
                    
                    $sqlDelete = "DELETE FROM agendamento_disponibilidade WHERE agendamento_id = " . $id;
                    $db->rawQuery($sqlDelete);
                    ...
                    $dataSource->rollback();
                    $this->Session->setFlash("Erro ao excluir registro da tabela de controle", 'flash_error');
                }            
    ...
    public function getExigencia($idUsuario = "", $numProcesso = ""){
        $this->layout = 'ajax';
        if ($this->request->is([ 'get', 'post'])) {
            ...
            $idTipologia =  $this->request['pass'][2];            
