...
class AgendamentoController extends BSController {
    ...
    public function carregarHorariosAtendimentoMunicipioEndereco() {
        if ($this->request->is(array('post', 'put'))) {
            ...
            $unidades = $this->UnidadeAtendimento->obterUnidadesCidMunicipio($idCID, true, $idMunicipio, false);
    ...
    public function montarListaHorariosDomicilio($intervaloConsulta, $agendasAtendimentos, $diaSemana) {
        ...
        foreach ($agendasAtendimentos as $agendaAtendimento) {
            ...
            foreach ($horas as $hora) {
                $horariosMedico = array_merge($horariosMedico, $this->formatarHorarioAtendimento($hora, $diaSemana));
            }
            if(!isset($horarios[$peritoID]['horarios']))$horarios[$peritoID]['horarios'] = [];
            $horarios[$peritoID]['horarios'] = array_merge($horarios[$peritoID]['horarios'], $horariosMedico);
            $horarios[$peritoID]['tipologias'] = Util::criarListaIds($agendaAtendimento['Tipologia']);
    ...
    private function formatarHorarioAtendimento($hora, $diaSemana, $agendaSistema = null) {
    ...
    */
    ...
    private function informarResponsaveis() {
        ...
        if ($chefiaUm) {
            ...
            if (isset($usuario1['Usuario']['email']) && !empty($usuario1['Usuario']['email'])) {
                $this->enviarEmailInformandoSolicitacaoLicenca($usuario1['Usuario']['email'], $this->request->data['Agendamento']['nome'], $this->request->data['Agendamento']['cpf'], $tipologia);
            // pr($usuario1);
            // pr('parou aqui');
            // die;
            }
        ...
        if ($chefiaDois) {
            ...
            if (isset($usuario2['Usuario']['email']) && !empty($usuario2['Usuario']['email'])) {
                $this->enviarEmailInformandoSolicitacaoLicenca($usuario2['Usuario']['email'], $this->request->data['Agendamento']['nome'], $this->request->data['Agendamento']['cpf'], $tipologia);
            }
        ...
        if ($chefiaTres) {
            ...
            if (isset($usuario3['Usuario']['email']) && !empty($usuario3['Usuario']['email'])) {
                $this->enviarEmailInformandoSolicitacaoLicenca($usuario3['Usuario']['email'], $this->request->data['Agendamento']['nome'], $this->request->data['Agendamento']['cpf'], $tipologia);
            }
    ...
    private function enviarEmailInformandoSolicitacaoLicenca($destinatario, $nomeServidor, $cpfServidor, $tipologia = '') {
    ...
    public function deletar($id, $retorno = false) {
        ...
        if ($this->request->is('get')) {
            ...
            else if($agendamento['Agendamento']['agendamento_confirmado'] && !in_array($tipologiaAgendamento, $tipologiasConfPres)){
