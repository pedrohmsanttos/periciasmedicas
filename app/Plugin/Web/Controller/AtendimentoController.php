...
public function consultarLaudoAtendimento() {
    ...
    public function consultarProcessosPublicacao() {
        ...
        if ($this->request->is('GET')) {
            ...
            $this->set('nome_usuario', $nomeUsuario);
            $this->set('cpf_usuario', $cpfUsuario);
    ...
    public function enviarPublicacao(){
        ...
        $cpf = $this->request->data['Usuario']['cpf'];
        ...
        $publicacoes = $this->listarProcessosPublicacao($dataIni, $dataFim,$nome_usuario,$cpf_usuario, false, true, $ids );
        // $data_ini, $data_fim, $nome_usuario = false,$cpf_usuario = false, $orgaoOrigem = false, $publicado = false, $ids = null
        // pr($publicacoes);die;
    ...
    public function index() {
        ...
        if ($tipoUsuarioLogado == USUARIO_PERITO_CREDENCIADO || $tipoUsuarioLogado == USUARIO_PERITO_SERVIDOR) {
            ...
            $arrAgendaDomicilio = $usuario['AgendaAtendimentoDomicilio'];
            //pr($arrAgendaDomicilio);die;
    ...
    private function carregarListaAtendimentosServidor() {
        ...
        $filtro->setCamposRetornadosString('Tipologia.id', 'Tipologia.nome', 'Atendimento.status_atendimento',
            'Atendimento.data_inclusao', 'TipoSituacaoParecerTecnico.nome', 'Atendimento.id', 'Atendimento.atedimento_pai_id', 'Atendimento.duracao');
        $joins = array();
        $joins[] = array(
            'table' => 'agendamento',
            'alias' => 'AgendamentoServidor',
    ...
    private function carregarListaSituacoes() {
        ...
        if($tipologia == TIPOLOGIA_ISENCAO_CONTRIBUICAO_PREVIDENCIARIA){
            ...
            $situacoes = Array('1' => 'Em Exigência','12' =>'Não se enquadra', '5' => 'Temporário', '6' => 'Definitivo');
    ...
    public function preVisualizarExcluido($id) {
        $this->Session->write("PROXIMO_ATENDIMENTO", $id);
        ...
        $atendimento = $this->Agendamento->find('first', array(
            'conditions' => array(
    ...
    public function atendimentoFinalizadoSucesso($id) {
        ...
        if(preg_match('#[^0-9]#',$id)== true){
            ...
            $this->Session->setFlash('Laudo inválido', 'flash_alert');
            return $this->redirect(array('controller' => 'Atendimento', 'action' => 'index'));
        ...
        if ($situacaoAtendimento == NULL){
            ...
            $result = $this->Session->setFlash(__('Este atendimento não existe!'), 'flash_alert');
            return $this->redirect(array('controller' => 'Atendimento', 'action' => 'index'));
    ...
    public function carregaAtendimentoCID($idAtendimento){
        ...
        $atendimentoNovo = $this->Atendimento->find('first', array(
            'conditions' => array(
                'Atendimento.id' => $idAtendimento
            )
        ));

        if($atendimentoNovo['Atendimento']['cid_id'] != ""){
            $cid = $this->Cid->findById($atendimentoNovo['Atendimento']['cid_id']);
    ...
    public function editar( $idAtendimento__ = null ){
        ...
        if ($this->request->is(array('post', 'put'))){
            ...
            unset($this->request->data['Atendimento']['data_inclusao']);
            // pr($this->request->data); die;
            ...
            if ($this->Atendimento->saveAll($this->request->data, $config)){
                ...
                if(!isset($this->request->data['Perito'])){
                    $this->loadModel('Usuario');
                    $idAtend = $this->request->data['Atendimento']['id'];                    
                    $this->Usuario->removeLastPerito($idAtend);
                }
        ...
        if($tipologiaAgendamento == TIPOLOGIA_EXAME_PRE_ADMISSIONAL){
            ...
            $ident = $this->request->data['Atendimento']['id'];
            $statusAtend = $this->request->data['Atendimento']['status_atendimento'];

            //pr($statusAtend);die;
            if($statusAtend =='Finalizado'){
                $this->Session->setFlash(__('Este Atendimento já foi finalizado!'), 'flash_alert');
                return $this->redirect(array('controller' => 'Atendimento', 'action' => 'index'));
            }else{
                $this->render('edit');    
            }
    ...
    private function limparSessoes($naoDelPerito = false) {

        if ($naoDelPerito == false){
    ...
    public function consultarAtendimentosPendentes() {
        ...
        if ($this->request->is('GET')) {
            ...
            // $publ = $this->Atendimento->listar($filtro);
            // $log = $this->Atendimento->getDataSource()->getLog(false, false);
            // pr($log);die;
            // die('ate aqui');
            // pr($publ);die;
    ...
    public function preEditarAtendimento($id) {
        ...
        $statusAtend = $atendimento['Atendimento']['status_atendimento'];
        //pr($situacao);die;
    ...
    // public function download_publicacao($id){
    //     die("oe");
    // }
    ...
    public function download_laudo($id) {
        ...
        if ($this->request->is('GET')) {
            ...
            $filtro->setCamposRetornadosString(
                ...
                'Agendamento.tratamento_acidente',
                'Agendamento.tratamento_acidente_processo',
                'Perito.nome',
                'Perito.numero_registro'
            );
            $joins = array();
            ...
            //pr($atendimento);die();
            $this->loadModel("Usuario");
            //$id_usuario = $this->request->data['Usuario']['id'];
            ...
            if($tipologiaId == TIPOLOGIA_RECURSO_ADMINISTRATIVO){
                ...
                $dataAtendimentoOrg = $this->Atendimento->findById($atendimento['AgendamentoServidor']['numero_processo'])['Atendimento']['data_inclusao'];
                $this->set('dataAtendimentoOrg', $dataAtendimentoOrg);
            ...
            if($atendimento['Tipologia']['id'] == TIPOLOGIA_RECURSO_ADMINISTRATIVO){
                ...
                $atendQGerouRecurso = $this->Atendimento->getAtendimentoByAgendamento($idAgendamentoOrig);
                // pr($atendQGerouRecurso);die;
                $this->set("dataAtend", $atendQGerouRecurso['data_parecer']);
            ...
            if(isset($atendimento['Agendamento']['tratamento_acidente']) && $atendimento['Agendamento']['tratamento_acidente'] == 1 ){
                $decorrenteCat = $atendimento['Agendamento']['tratamento_acidente_processo'];
                $this->set('decorrenteCat', $decorrenteCat);
            }

            $this->set('dataProtocolo', $atendimento['Agendamento']['data_inclusao']);
            ...
            $dataHoje = date('Y-m-d');
            $horaLaudo = date('H:i:s');
            //$id_usuario = 
            $url_atual = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
            
            $status_Atend = $atendimento['Atendimento']['status_atendimento'];
            //pr($status_Atend);die;
            if($status_Atend == 'Finalizado'){
                //Comentado no dia 22/01/2019 - #21087

                // $result = $this->Atendimento->setDownloadLaudo($id,$dataHoje,$url_atual,$horaLaudo);
            }else{
                $this->Session->setFlash(__('Este atendimento ainda não foi finalizado!'), 'flash_alert');
                return $this->redirect(array('controller' => 'Atendimento', 'action' => 'index'));
            }
    ...
    private function informaArquivoLaudo($idTipologia, $idTipo='') {
        ...
        switch ($idTipologia) {
            ...
            case TIPOLOGIA_COMUNICACAO_DE_ACIDENTE_DE_TRABALHO:
                ...
                break;
            case TIPOLOGIA_APOSENTADORIA_ESPECIAL:
                $this->set('tituloLaudo', 'Aposentadoria Especial');
    ...
    private function listarProcessosPublicacao($data_ini, $data_fim, $nome_usuario = false,$cpf_usuario = false, $orgaoOrigem = false, $publicado = false, $ids = null){
        ...
        if(!empty($cpf_usuario) && $cpf_usuario){
        ...
        // if(!($vin_ids['0'] == "" && $vin_ids['1'] == "0" && $vin_ids['2'] == "0")){
            $condicoes['Vinculo.id in '] = $vin_ids;
        // }
        
        $filtroC->setCondicoes($condicoes);
        $filtroC->setTipo('all');
        //$filtroB->setCamposOrdenados(array('OrgaoOrigem.orsgao_origem'=>'asc','ParecerSituacao.nome'=>'asc', 'Tipologia.id'=>'asc','Usuario.nome'=>'asc'));

        $filtroC->setCamposAgrupados(array('OrgaoOrigem.orgao_origem', 'Tipologia.nome', 'TipologiaRecurso.nome', 'Atendimento.modo', 'ParecerSituacao.nome','OrgaoOrigem.id', 'ParecerSituacao.id', 'Tipologia.id',
        ...
        //$filtro->setLimiteConsulta($limitConsulta);
        $filtroC->setCamposOrdenados(array('OrgaoOrigem.orgao_origem'=>'asc','ParecerSituacao.nome'=>'asc', 'Tipologia.id'=>'asc', 'Atendimento.modo'=>'asc','Usuario.nome'=>'asc'));
        $filtroC->setCamposRetornados(
            [
                'OrgaoOrigem.orgao_origem',
                'ParecerSituacao.nome',
                'Tipologia.nome',
        ...
        $lista = $this->Atendimento->listar($filtroC);
    ...
    public function consultarPublicacoes(){
        ...
        if ($this->request->is('GET')) {    
            ...
            if(!empty($nomeUsuario)){
                $options['conditions']['UsuarioVersao.nome ILIKE '] = "%".$nomeUsuario."%";
                $condicao['Usuario.nome ILIKE '] = "%".$nomeUsuario."%";
            }

            if(!empty($cpfUsuario)){
               $options['conditions']['UsuarioVersao.cpf'] = $cpfUsuario;
               $condicao['Usuario.cpf'] = $cpfUsuario;
            }
            ...
            // $publicacoes = $this->Publicacao->listar($filtro);
            $publicacoes = $this->paginar($filtro, 'Publicacao');
